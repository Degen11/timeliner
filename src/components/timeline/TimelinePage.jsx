    ...
    >
      <div className="flex items-center gap-2 rounded-lg bg-gray-100/80 dark:bg-white/[0.06] border border-gray-200/60 px-3 py-2 mb-3 text-xs">
        ...
        <span className="text-text-muted shrink-0">
          ...
          of <AnimatedCount value={totalCount} /> event{totalCount !== 1 ? 's' : ''}
        </span>
        <div className="flex items-center gap-1.5 flex-wrap flex-1 min-w-0">
          ...
          {filters.people.map((p) => (
            <span
              key={p}
              className="inline-flex items-center gap-1 rounded-full bg-surface px-2 py-0.5 text-xs text-text-muted border border-gray-200/60"
            >
              {p}
              <button
                onClick={() =>
          ...
          {hydrating ? (
            ...
            >
              ...
              {[1, 2, 3].map((i) => (
                <div
                  key={i}
                  className="rounded-xl bg-surface border border-gray-200/60 p-5 space-y-3"
                >
            ...
            >
              <AnimatePresence>{showWelcome && <WelcomeBanner events={events} />}</AnimatePresence>
                    ...
                    >
                      {activeView === VIEWS.HORIZONTAL && (
                        <div className="sm:hidden mb-3 rounded-lg bg-amber-50 border border-amber-200 px-3 py-2.5 text-xs text-amber-800 flex items-center justify-between gap-2">
                          ...
                          <button
                            onClick={() =>
                              useTimelineStore.getState().setActiveView(VIEWS.VERTICAL)
                            }
                    ...
                    <div className="flex justify-center py-4">
                      ...
                      >
                        {isLoadingMore ? (
                          ...
                          <>
                            Load more (<AnimatedCount value={sorted.length - paginated.length} />{' '}
                            remaining)
                          </>
                        )}
                      </Button>
